...
func UpdateSubject(conn *pgxpool.Pool, subject Subject) error {
	x, err := conn.Exec(context.Background(),
		"UPDATE subject SET name=$1, age=$2 WHERE id=$3",
		subject.Name, subject.Age, subject.ID,
	)
	if err != nil {
		return fmt.Errorf("Subject güncellenemedi: %w", err)
	}
	if x.RowsAffected() == 0 {
...
func DeleteSubjectByID(conn *pgxpool.Pool, id string) error {
	x, err := conn.Exec(context.Background(), "DELETE FROM subject WHERE id=$1", id)
	if err != nil {
...
func GetSubjectByID(conn *pgxpool.Pool, id string) (Subject, error) {
	var s Subject
	err := conn.QueryRow(context.Background(), "SELECT id, name, age FROM subject WHERE id=$1", id).Scan(&s.ID, &s.Name, &s.Age)
	if err != nil {
		return s, fmt.Errorf("Subject bulunamadı: %w", err)
	}
	return s, nil
}